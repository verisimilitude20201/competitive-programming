...
Approach:
--------
1. There can be utmost two minimum height trees that can be constructed from a graph. 
2. A leaf node with a degree of 1 cannot contribute to an MHT.
3. We can prune the leaves of the tree one by one decrementing the degree of each of it's adjacent nodes. 
4. We stop this process when the total number of remaining nodes is 2 or 1. This hypothesis is from point 1, there can be utmost 2 minimum height trees for
a given graph.
...
def find_minimum_height_trees(nodes, edges):
    if nodes <= 0:
        return []
...
def main():
    print("Roots of MHTs: " +
          str(find_minimum_height_trees(5, [[0, 1], [1, 2], [1, 3], [2, 4]])))
    #print("Roots of MHTs: " +
